...
<script type="text/javascript">
		...
		// alert(name);
			$.ajax({
				type:"post",
				url : "${pageContext.request.contextPath}/sellermenu/sel/"+name,
				success : function(result) {
					...
					for (var i = 0; i < menus.length; i++) {
						...
						dis=dis+"<img width='30px' height='80px' src='upload/"+menus[i].pic+"' style='width:100%;'></div>";
						...
						//dis=dis+"<span class='line-right'> <input type='button' value='删除菜品'  onclick='addCart("+menus[i].id+")'> </span>";
						dis=dis+"<span class='line-right'> <input type='image' src='img/del.png' border='0' width=20px height=20px' onclick='deletemenu("+menus[i].id+")'> </span>";
		...
		function deletemenu(menuid) {
			$.ajax({
				url : "${pageContext.request.contextPath}/seller/islogin",
				success : function(result) {
					...
					if(result=="mtc_null"){
						...
						$.ajax({
							url : "${pageContext.request.contextPath}/sellermenu/delete/"+menuid,
	...
	function	logout(){
		$.ajax({
			url : "${pageContext.request.contextPath}/seller/logout",
			success : function(result) {
			}
		});
	}
		...
		$.ajax({
			...
			success : function(result) {
				...
				if(result=="mtc_null"){
					...
					window.location.href="${pageContext.request.contextPath}/uploadmenu";
				}
			}
		});
	}
