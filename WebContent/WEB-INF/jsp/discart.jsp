...
.navbar .nav-dropdown:before {
	...
	<div align="center">
		<br> <br> <br> <br>
		 <a href="${pageContext.request.contextPath}/member/login" style=""><button
				style="width: 20%;" type="submit" id="login"
				class="btn btn-normal btn-block disabled mj_login login-btn">登录</button></a>
		 <a href="login.html" style=""><button  onclick="logout()"
				style="width: 20%;" type="submit" id="login"
				class="btn btn-normal btn-block disabled mj_login login-btn">注销</button></a>
		 <a href="${pageContext.request.contextPath}/member/register"><button style="width: 20%;" type="submit" id="login"
				class="btn btn-normal btn-block disabled mj_login login-btn">注册</button></a>
		 <a href="${pageContext.request.contextPath}/getmenu"><button style="width: 30%;" type="submit" id="login"
				class="btn btn-normal btn-block disabled mj_login login-btn">继续购物</button></a>			
	</div>

</body>
<script type="text/javascript">
        //点击搜索按钮，找到id为keyword对应的值的那个菜
		function getCart() {
			...
			$.ajax({
				type : "post",
				url : "${pageContext.request.contextPath}/cart?name=" + name,
				success : function(result) {
					...
					for (var i = 0; i < menus.length; i++) {
						...
						dis=dis+"<img width='30px' height='80px' src='${pageContext.request.contextPath}/images/"+menus[i].pic+"' style='width:100%;'></div>";
						dis=dis+"<div class='dealcard-block-right'>";
						dis=dis+"<div class='dealcard-brand single-line'>"+menus[i].name+"</div>";
						dis=dis+"<div class='dealcard-brand single-line'>"+menus[i].remark+"</div>";
						dis=dis+"<div class='price'>";
						dis=dis+"<span class='strong'>"+menus[i].price+"</span><span class='strong-color'>元</span>";
						dis=dis+"<span class='line-right'> <input type='image' src='img/del.png' border='0' width=20px height=20px' onclick='deleteCart("+menus[i].id+")'> </span>";
		...
		$.ajax({
			...
			success : function(result) {
				...
				if(result=="htc_null"){
					...
					$.ajax({
						url : "${pageContext.request.contextPath}/cart/delete?menuid="+menuid,
